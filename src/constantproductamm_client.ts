...
export default class ConstantProductAMM extends ApplicationClient {
    desc: string = "";
    appSchema: Schema = {
        declared: {
    ...
    mint(a_xfer: algosdk.TransactionWithSigner | algosdk.Transaction, b_xfer: algosdk.TransactionWithSigner | algosdk.Transaction, pool_asset: number, a_asset: number, b_asset: number) {
        return this.call(algosdk.getMethodByName(this.methods, "mint"), { a_xfer: a_xfer, b_xfer: b_xfer, pool_asset: pool_asset, a_asset: a_asset, b_asset: b_asset });
    }
    burn(pool_xfer: algosdk.TransactionWithSigner | algosdk.Transaction, pool_asset: number, a_asset: number, b_asset: number) {
        return this.call(algosdk.getMethodByName(this.methods, "burn"), { pool_xfer: pool_xfer, pool_asset: pool_asset, a_asset: a_asset, b_asset: b_asset });
    }
    set_governor(new_governor: string) {
        return this.call(algosdk.getMethodByName(this.methods, "set_governor"), { new_governor: new_governor });
    }
    bootstrap(seed: algosdk.TransactionWithSigner | algosdk.Transaction, a_asset: number, b_asset: number) {
        return this.call(algosdk.getMethodByName(this.methods, "bootstrap"), { seed: seed, a_asset: a_asset, b_asset: b_asset });
    }
    swap(swap_xfer: algosdk.TransactionWithSigner | algosdk.Transaction, a_asset: number, b_asset: number) {
        return this.call(algosdk.getMethodByName(this.methods, "swap"), { swap_xfer: swap_xfer, a_asset: a_asset, b_asset: b_asset });
    }
}
