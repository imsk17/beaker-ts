...
class InsecureWallet extends Wallet {
  ...
  override async sign(txns: Transaction[]): Promise<SignedTxn[]> {
    ...
    for (const txidx in txns) {
      ...
      const sk = this.pkToSk[addr];
      if (sk !== undefined && addr === defaultAddr) {
        signed.push({ txID: txn.txID(), blob: txn.signTxn(sk) });
      } else {
