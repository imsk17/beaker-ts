...
class AlgoSignerWallet extends Wallet {
  ...
  async signTxn(txns: Transaction[]): Promise<SignedTxn[]> {
    ...
    const encodedTxns = txns.map((tx: Transaction) => {
      const t: { txn: string; signers?: string[] } = {
    ...
    return signed.map((signedTx: { txID: string; blob: string }) => {
      if (signedTx)
        return {
          txID: signedTx.txID,
